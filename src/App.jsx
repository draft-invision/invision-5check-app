...
export default function App(){
  ...
  useEffect(function(){(async function(){
    try{var r=await window.storage.get("iv5-user");if(r&&r.value){var d=JSON.parse(r.value);setUd(d);setEmail(d.email||"");setScreen(d.name&&d.habitHistory&&d.habitHistory.length>0?"check":"setup");}}catch(e){}
    try{var m=await window.storage.get("iv5-members");if(m&&m.value)setMembers(JSON.parse(m.value));}catch(e){}
    try{var t=await window.storage.get("iv5-teams");if(t&&t.value)setTeamsS(JSON.parse(t.value));}catch(e){}
    setLoading(false);
  })();},[]);

  var sU=async function(d){setUd(d);try{await window.storage.set("iv5-user",JSON.stringify(d));}catch(e){}};
  var sM=async function(m){setMembers(m);try{await window.storage.set("iv5-members",JSON.stringify(m));}catch(e){}};
  var sT=async function(t){setTeamsS(t);try{await window.storage.set("iv5-teams",JSON.stringify(t));}catch(e){}};
  var sync=function(u){var idx=members.findIndex(function(m){return m.email===u.email;});if(idx>=0){var ms=members.map(function(m,i){return i===idx?{...m,checks:u.checks,habitHistory:u.habitHistory}:m;});sM(ms);}};
  ...
  var hLogout=async function(){try{await window.storage.delete("iv5-user");}catch(e){}setUd(null);setEmail("");setScreen("login");};
...
function SetupScr(p){
  ...
  return(
    <div style={st.mn}><div style={{maxWidth:560,margin:"0 auto",animation:"fadeIn 0.5s"}}>
...
function CheckScr(p){
  ...
  return(
    <div style={st.mn}><div style={{maxWidth:700,margin:"0 auto"}}>
...
function HabitsScr(p){
  ...
  return(
    <div style={st.mn}><div style={{maxWidth:600,margin:"0 auto"}}>
...
function DashScr(p){
  ...
  return(
    <div style={st.mn}><div style={{maxWidth:860,margin:"0 auto"}}>
...
var st={
  ...
  input:{width:"100%",padding:"7px 10px",border:"2px solid #e8e3d8",borderRadius:5,fontSize:12,fontFamily:FF,outline:"none",marginBottom:5,background:"#faf7f0"},
