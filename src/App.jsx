...
import darumaPng from "./assets/daruma.png";
  ...
  return(
    <div style={{position:"relative",display:"inline-flex",flexDirection:"column",alignItems:"center",flexShrink:0}}>
      ...
      <img
        ...
        alt="mascot"
        style={{width:120,height:"auto",animation:"birdFly 2s ease-in-out infinite",display:"block"}}
      />
    </div>
  );
}
...
function NorenBar(){
  return <div style={{width:"100%",height:80,overflow:"hidden",lineHeight:0}}><img src={framePng} alt="" style={{width:"100%",height:"auto",display:"block"}}/></div>;
}

function FooterBar(){
  return <div style={{width:"100%",lineHeight:0,fontSize:0}}>
    <img src={barPng} alt="" style={{width:"100%",display:"block",margin:0,padding:0}}/>
    <img src={framePng} alt="" style={{width:"100%",display:"block",height:80,objectFit:"cover",objectPosition:"bottom",margin:0,padding:0}}/>
  </div>;
}
    ...
    <div style={{fontFamily:FF,background:"linear-gradient(180deg,#F5F0E1,#EDE7D6)",minHeight:"100vh",color:"#333"}}>
      ...
      {screen==="login"&&(
        <div style={st.ctr}><div style={st.lc}>
          <div style={{textAlign:"center",marginBottom:14}}><NorenBar/><div style={{marginTop:10}}><img src={HIDANE} alt="" width="40" height="40"/></div><h1 style={{fontSize:20,fontWeight:900,color:"#1B4B8A",margin:"6px 0 2px"}}>行動習慣JOBTRIP</h1><p style={{fontSize:10,color:"#8B7E6A",margin:0}}>インビジョン株式会社</p></div>
    ...
    <div style={st.mn} className="iv-mn"><div>
      ...
      <div style={{...st.cd,marginTop:14,textAlign:"center"}}>
        ...
        <div style={{position:"absolute",left:8,top:"50%",transform:"translateY(-50%)",zIndex:5}}>
          <style>{"@keyframes darumaFloat{0%,100%{transform:translateY(0px) rotate(-45deg)}50%{transform:translateY(-12px) rotate(-45deg)}}"}</style>
          <img src={darumaPng} alt="" style={{width:90,height:"auto",animation:"darumaFloat 2s ease-in-out infinite",display:"block"}}/>
        </div>
      </div>
    </div></div>
  );
